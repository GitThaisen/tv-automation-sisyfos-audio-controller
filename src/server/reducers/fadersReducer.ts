...
const defaultFadersReducerState = (numberOfFaders: number): IFaders[] => {
	const defaultObj: Array<IFaders> = [
		{
			fader: [],
			vuMeters: []
		}
...
export const faders = (state = defaultFadersReducerState(0), action: any): Array<IFaders> => {
	...
	switch (action.type) {
		...
		case SET_COMPLETE_FADER_STATE: //allState  //numberOfChannels
			const emptyState = defaultFadersReducerState(action.numberOfTypeChannels)
		...
		case IGNORE_AUTOMATION: //channel // ignoreAutomation
			nextState[0].fader[action.channel].ignoreAutomation = !nextState[0].fader[action.channel]
				.ignoreAutomation
			return nextState
		case TOGGLE_SNAP: //channel //snapIndex
			nextState[0].fader[action.channel].snapOn[action.snapIndex] = !nextState[0].fader[
				action.channel
			].snapOn[action.snapIndex]
			return nextState
		case X_MIX: //none
			nextState[0].fader.map((item, index) => {
				const nextPgmOn = state[0].fader[index].pstOn
				const nextVoOn = state[0].fader[index].pstVoOn

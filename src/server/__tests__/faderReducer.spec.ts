...
const fs = require('fs')
const parsedSimpleStoreJSON = fs.readFileSync('server/__tests__/__mocks__/parsedSimpleStore.json')
...
describe('Test redux faderReducers actions', () => {
	...
	it('should return the new fader level state on faders', () => {
		const parsedInitialStore = JSON.parse(parsedSimpleStoreJSON)
	...
	it('should SET PST ch 10-14 and return the new CLEAR_PST state on faders', () => {
		...
		const newState = JSON.parse(parsedFullStoreJSON)
	...
	it('should SET PST_VO ch 10-14 and set PST ch 6-8 and return the new X_MIX state on faders twice', () => {
		...
		const finalState = JSON.parse(parsedFullStoreJSON)
		for (let i = 10; i < 14; i++) {
	...
	it('should HIDE ch 10 and return the new SHOW_CHANNEL state on faders', () => {
		const parsedFullStore = JSON.parse(parsedFullStoreJSON)
		const newState = JSON.parse(parsedFullStoreJSON)

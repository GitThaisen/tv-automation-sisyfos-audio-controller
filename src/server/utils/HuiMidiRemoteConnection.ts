...
import { SET_FADER_LEVEL, TOGGLE_PGM, TOGGLE_PFL } from '../reducers/faderActions'
...
export class HuiMidiRemoteConnection {
	...
	constructor() {
		...
		this.mixerProtocol =
			MixerProtocolPresets[state.settings[0].mixerProtocol] || MixerProtocolPresets.genericMidi
	...
	setupRemoteFaderConnection() {
		this.midiInput.addListener(
			this.midiReceiveTypes[this.remoteProtocol.fromRemote.CHANNEL_FADER_LEVEL.type],
			undefined,
			(message: any) => {
				if (message.data[1] < 9) {
					//Fader changed:
	...
	convertToRemoteLevel(level: number) {
		const oldMin = this.mixerProtocol.fader.min
		const oldMax = this.mixerProtocol.fader.max
		const newMin = this.remoteProtocol.fader.min
		const newMax = this.remoteProtocol.fader.max

		const indexLevel = (level / (oldMax - oldMin)) * (newMax - newMin)
		const newLevel = newMin + indexLevel
		return newLevel //convert from mixer min-max to remote min-max
	}

	convertFromRemoteLevel(level: number) {
		const oldMin = this.remoteProtocol.fader.min
		const oldMax = this.remoteProtocol.fader.max
		const newMin = this.mixerProtocol.fader.min
		const newMax = this.mixerProtocol.fader.max
	...
	updateRemoteFaderState(channelIndex: number, outputLevel: number) {
		...
		this.midiOutput.sendControlChange(channelIndex, this.convertToRemoteLevel(outputLevel), 1)

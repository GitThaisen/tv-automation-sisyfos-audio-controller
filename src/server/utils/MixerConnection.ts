...
export class MixerGenericConnection {
	...
	constructor() {
		...
		// Get mixer protocol
		this.mixerProtocol =
			MixerProtocolPresets[state.settings[0].mixerProtocol] || MixerProtocolPresets.sslSystemT
	...
	updateOutLevel(faderIndex: number, fadeTime = -1) {
		if (fadeTime === -1) {
	...
	updateNextAux(faderIndex: number) {
		...
		if (state.faders[0].fader[faderIndex].pstOn) {
			...
			level =
				(state.faders[0].fader[faderIndex].faderLevel * (100 - state.settings[0].voLevel)) / 100
		}
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateThreshold(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].threshold
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateRatio(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].ratio
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateDelayTime(faderIndex: number) {
		const delayTime = state.faders[0].fader[faderIndex].delayTime
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateLow(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].low
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateLoMid(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].loMid
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateMid(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].mid
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {
	...
	updateHigh(faderIndex: number) {
		const level = state.faders[0].fader[faderIndex].high
		state.channels[0].channel.map((channel: IChannel, channelIndex: number) => {
			if (faderIndex === channel.assignedFader) {

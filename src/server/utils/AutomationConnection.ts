...
export class AutomationConnection {
			...
			.on('message', (message: any, timetag: number | undefined, info: any) => {
				...
				if (
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.CHANNEL_PGM_ON_OFF
					)
				) {
					const ch = message.address.split('/')[2]
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.CHANNEL_PST_ON_OFF
					)
				) {
					const ch = message.address.split('/')[2]
					if (!state.faders[0].fader[ch - 1].ignoreAutomation) {
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.CHANNEL_MUTE)
				) {
					const ch = message.address.split('/')[2]
					if (!state.faders[0].fader[ch - 1].ignoreAutomation) {
						if (message.args[0] === 1) {
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.CHANNEL_FADER_LEVEL
					)
				) {
					const ch = message.address.split('/')[2]
					if (!state.faders[0].fader[ch - 1].ignoreAutomation) {
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.INJECT_COMMAND
					)
				) {
					const ch = message.address.split('/')[2]
					store.dispatch({
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.SNAP_RECALL)
				) {
					const snapNumber = message.address.split('/')[2]
					store.dispatch({
						type: SNAP_RECALL,
						snapIndex: snapNumber - 1
					})
				} else if (
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.SET_LABEL)
				) {
					const ch = message.address.split('/')[2]
					store.dispatch({
						type: SET_CHANNEL_LABEL,
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.X_MIX)
				) {
					store.dispatch({
						type: X_MIX
					})
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.CHANNEL_VISIBLE
					)
				) {
					const ch = message.address.split('/')[2]
					store.dispatch({
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.CLEAR_PST)
				) {
					store.dispatch({
						type: CLEAR_PST
					})
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.STATE_FULL)
				) {
					this.sendOutMessage(
						...
						JSON.stringify({
							channel: state.faders[0].fader.map(
								({ faderLevel, pgmOn, voOn, pstOn, showChannel }: IFader) => ({
									faderLevel,
									pgmOn,
									voOn,
									pstOn,
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.STATE_CHANNEL_PGM
					)
				) {
					const ch = message.address.split('/')[3]
					this.sendOutMessage(
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.STATE_CHANNEL_PST
					)
				) {
					const ch = message.address.split('/')[3]
					this.sendOutMessage(
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.STATE_CHANNEL_MUTE
					)
				) {
					const ch = message.address.split('/')[3]
					this.sendOutMessage(
					...
					this.checkOscCommand(
						message.address,
						this.automationProtocol.fromAutomation.STATE_CHANNEL_FADER_LEVEL
					)
				) {
					const ch = message.address.split('/')[3]
					this.sendOutMessage(
					...
					this.checkOscCommand(message.address, this.automationProtocol.fromAutomation.PING)
				) {
					const pingValue = state.settings[0].mixerOnline
						? message.address.split('/')[2]
						: 'offline'

					this.sendOutMessage(this.automationProtocol.toAutomation.PONG, 0, pingValue, 's', info)
				}
			})
			.on('error', (error: any) => {
	...
	checkOscCommand(message: string, command: string) {
		...
		const cmdArray = command.split('{value1}')
	...
	sendOutMessage(
		...
		const channelString = this.automationProtocol.leadingZeros
			? ('0' + channel).slice(-2)
			: channel.toString()
		const message = oscMessage.replace('{value1}', channelString)
		if (message != 'none') {

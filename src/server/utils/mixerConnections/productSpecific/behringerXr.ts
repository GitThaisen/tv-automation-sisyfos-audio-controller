...
export const behringerXrMeter = (message: any) => {
	...
	const uint8bytes = Uint8Array.from(message[0])
	const dataview = new DataView(uint8bytes.buffer)

	for (let i = 0; i < state.settings[0].numberOfChannelsInType[0]; i++) {
		const level = (dataview.getInt16(DATA_OFFSET + 2 * i, true) + 8000) / 8000
		store.dispatch({
			type: SET_VU_LEVEL,
			channel: i,
			level: level
...
export const behringerReductionMeter = (message: any) => {
	...
	for (let i = 0; i < state.settings[0].numberOfChannelsInType[0]; i++) {
		const level = 1 - (dataview.getInt16(DATA_OFFSET + 2 * (i + 16), true) + 8000) / 8000
		store.dispatch({

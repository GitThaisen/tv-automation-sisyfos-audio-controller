...
*/
	...
	updateOutLevel(channelIndex: number) {
		...
		const protocol = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_OUT_GAIN[0]
		const level =
			(state.channels[0].channel[channelIndex].outputLevel - protocol.min) *
	...
	updateFadeIOLevel(channelIndex: number, outputLevel: number) {
		...
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const protocol = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_OUT_GAIN[0]
		const level = (outputLevel - protocol.min) * (protocol.max - protocol.min)

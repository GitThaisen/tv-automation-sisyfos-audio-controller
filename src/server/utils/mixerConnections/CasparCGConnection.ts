...
export class CasparCGConnection {
	...
	setupMixerConnection() {
		if (!this.oscClient) {
					...
					if (index !== undefined && message.args) {
						...
						if (m[1] === 'channel' && m[6] === 'producer' && m[7] === 'type') {
							const index = this.mixerProtocol.sourceOptions.sources.findIndex(
								(i) => i.channel === parseInt(m[2], 10) && i.layer === parseInt(m[5])
							)
							if (index >= 0) {
								store.dispatch({
									type: SET_PRIVATE,
							...
							if (index >= 0) {
								const value =
									typeof message.args[0] === 'string' ? message.args[0] : message.args[0].low
								store.dispatch({
									type: SET_PRIVATE,
									channel: index,
									tag: 'file_path',
			...
			this.oscClient.open()
			logger.info(
				'Listening for status on CasparCG: ' + state.settings[0].deviceIp + ':' + remotePort
			)
		}
	...
	updatePflState(channelIndex: number) {
		...
		if (state.faders[0].fader[channelIndex].pflOn === true) {
			...
			const others = state.faders[0].fader
				...
				.filter(
					(i) =>
						i !== undefined && i < this.mixerProtocol.toMixer.MONITOR_CHANNEL_FADER_LEVEL.length
				) as number[]
			others.forEach((i) => {

...
export class OscMixerConnection {
			...
			.on('message', (message: any) => {
				...
				if (
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_VU[0].mixerMessage
					)
				) {
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_VU_REDUCTION[0].mixerMessage
					)
				) {
					if (state.settings[0].mixerProtocol.includes('behringer')) {
					...
					const ch = message.address.split('/')[this.cmdChannelIndex]
					const assignedFaderIndex = state.channels[0].channel[ch - 1].assignedFader

					if (!state.channels[0].channel[ch - 1].fadeActive) {
						if (
							message.args[0] >
							this.mixerProtocol.fader.min +
								(this.mixerProtocol.fader.max * state.settings[0].autoResetLevel) / 100
						) {
							store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.AUX_LEVEL[0].mixerMessage
					)
				) {
					const commandArray: string[] = this.mixerProtocol.channelTypes[0].fromMixer.AUX_LEVEL[0].mixerMessage.split(
						'/'
					)
					const messageArray: string[] = message.address.split('/')
					let ch = 0
					let auxIndex = 0
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_NAME[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_MUTE_ON[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					const mute = message.args[0] === 0 ? 1 : 0
					store.dispatch({
						type: SET_MUTE,
						channel: state.channels[0].channel[ch - 1].assignedFader,
						muteOn: mute
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.THRESHOLD[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.RATIO[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					const ratio = this.mixerProtocol.channelTypes[0].fromMixer.RATIO[0]
					const level = message.args[0] / (ratio.max - ratio.min + ratio.min)
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.DELAY_TIME[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					const delay = this.mixerProtocol.channelTypes[0].fromMixer.DELAY_TIME[0]
					const delayTime = message.args[0] / (delay.max - delay.min + delay.min)
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.LOW[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.LO_MID[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.MID[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
					...
					this.checkOscCommand(
						message.address,
						this.mixerProtocol.channelTypes[0].fromMixer.HIGH[0].mixerMessage
					)
				) {
					const ch = message.address.split('/')[this.cmdChannelIndex]
					store.dispatch({
								...
								if (channel.assignedFader >= 0) {
									setTimeout(() => {
										this.sendOutRequestAux(
											item.mixerMessage,
											auxIndex + 1,
											state.faders[0].fader[channel.assignedFader].monitor
										)
	...
	pingMixerCommand() {
		...
		this.mixerProtocol.pingCommand.map((command) => {
			this.sendOutMessage(command.mixerMessage, 0, command.value, command.type)
		})
		global.mainThreadHandler.updateFullClientStore()
	...
	sendOutMessage(oscMessage: string, channel: number, value: string | number, type: string) {
		const channelString = this.mixerProtocol.leadingZeros
			? ('0' + channel).slice(-2)
			: channel.toString()
		const message = oscMessage.replace('{channel}', channelString)
		if (message != 'none') {
	...
	sendOutRequestAux(oscMessage: string, channel: number, auxSend: number) {
		...
		const auxSendNumber = this.mixerProtocol.leadingZeros
	...
	updateOutLevel(channelIndex: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		this.sendOutMessage(
	...
	updateMuteState(channelIndex: number, muteOn: boolean) {
		...
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		if (muteOn === true) {
			const mute = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_MUTE_ON[0]
			this.sendOutMessage(mute.mixerMessage, channelTypeIndex + 1, mute.value, mute.type)
		} else {
			const mute = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_MUTE_OFF[0]
			this.sendOutMessage(mute.mixerMessage, channelTypeIndex + 1, mute.value, mute.type)
		}
	}
	...
	updateThreshold(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const thr = this.mixerProtocol.channelTypes[channelType].toMixer.THRESHOLD[0]
	...
	updateRatio(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const ratio = this.mixerProtocol.channelTypes[channelType].toMixer.RATIO[0]
		level = level * (ratio.max - ratio.min) + ratio.min
		this.sendOutMessage(ratio.mixerMessage, channelTypeIndex + 1, level, 'f')
	}
	updateDelayTime(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const delayTime = this.mixerProtocol.channelTypes[channelType].toMixer.DELAY_TIME[0]
		level = level * (delayTime.max - delayTime.min) + delayTime.min
		this.sendOutMessage(delayTime.mixerMessage, channelTypeIndex + 1, level, 'f')
	}
	updateLow(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const low = this.mixerProtocol.channelTypes[channelType].toMixer.LOW[0]
	...
	updateLoMid(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const loMid = this.mixerProtocol.channelTypes[channelType].toMixer.LO_MID[0]
		level = level * (loMid.max - loMid.min) + loMid.min
		this.sendOutMessage(loMid.mixerMessage, channelTypeIndex + 1, level, 'f')
	}
	updateMid(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const mid = this.mixerProtocol.channelTypes[channelType].toMixer.MID[0]
	...
	updateHigh(channelIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const high = this.mixerProtocol.channelTypes[channelType].toMixer.HIGH[0]
	...
	updateAuxLevel(channelIndex: number, auxSendIndex: number, level: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channel = state.channels[0].channel[channelIndex].channelTypeIndex + 1
		const auxSendCmd = this.mixerProtocol.channelTypes[channelType].toMixer.AUX_LEVEL[0]
		const auxSendNumber = this.mixerProtocol.leadingZeros
			? ('0' + String(auxSendIndex + 1)).slice(-2)
			: String(auxSendIndex + 1)
		const message = auxSendCmd.mixerMessage.replace('{argument}', auxSendNumber)
	...
	updateChannelName(channelIndex: number) {
		const channelType = state.channels[0].channel[channelIndex].channelType
		const channelTypeIndex = state.channels[0].channel[channelIndex].channelTypeIndex
		const channelName = state.faders[0].fader[channelIndex].label
		this.sendOutMessage(

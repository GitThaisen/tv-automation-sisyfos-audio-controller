...
import { SET_FADER_LEVEL, TOGGLE_PGM, SET_MUTE } from '../../reducers/faderActions'
...
export class SSLMixerConnection {
			...
			.on('data', (data: any) => {
				...
				buffers.forEach((buffer) => {
					if (buffer[1] === 6 && buffer[2] === 255 && !lastWasAck) {
						...
						// FADERLEVEL COMMAND:
						try {
							const commandHex = buffer.toString('hex')
							const channel = buffer[6]
							const value = buffer.readUInt16BE(7) / 1024

							const assignedFaderIndex = state.channels[0].channel[channel].assignedFader
						...
						// MUTE ON/OFF COMMAND
						const commandHex = buffer.toString('hex')
						const channelIndex = buffer[6]
						const value: boolean = buffer[7] === 0 ? true : false
						logger.verbose(
							'Receive Buffer Channel On/off: ' + this.formatHexWithSpaces(commandHex, ' ', 2),
							{}
						)

						const assignedFaderIndex = state.channels[0].channel[channelIndex].assignedFader
						...
						const commandHex = buffer.toString('hex')
						logger.verbose(
							'Receieve Buffer Hex: ' + this.formatHexWithSpaces(commandHex, ' ', 2),
							{}
						)
					}
					if (buffer[0] === 4) {
	...
	calculate_checksum8(hexValues: string) {
		...
		const strHex = new String('0123456789ABCDEF')
		let result = 0
		let fctr = 16
		...
		for (let i = 0; i < hexValues.length; i++) {
			...
			const v = strHex.indexOf(hexValues.charAt(i))
			if (v < 0) {
				result = -1
				break
			}
	...
	sendOutLevelMessage(sslMessage: string, channelIndex: number, value: string | number) {
		...
		const channelByte = new Uint8Array([
	...
	updateOutLevel(channelIndex: number) {
		...
		const faderIndex = state.channels[0].channel[channelIndex].assignedFader

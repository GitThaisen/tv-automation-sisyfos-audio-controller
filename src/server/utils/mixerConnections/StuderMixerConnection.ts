...
export class StuderMixerConnection {
	...
	constructor(mixerProtocol: IMixerProtocol) {
		...
		this.emberConnection = new EmberClient(state.settings[0].deviceIp, state.settings[0].devicePort)
	...
	subscribeFaderLevel(ch: number, typeIndex: number, channelTypeIndex: number) {
		const command = this.mixerProtocol.channelTypes[
			typeIndex
		].fromMixer.CHANNEL_OUT_GAIN[0].mixerMessage.replace('{channel}', String(channelTypeIndex + 1))
		this.emberConnection
			...
			.then((node: any) => {
				...
				this.emberConnection.subscribe(node, () => {
					...
					if (
						...
						!state.channels[0].channel[ch - 1].fadeActive &&
						node.contents.value >
							this.mixerProtocol.channelTypes[typeIndex].fromMixer.CHANNEL_OUT_GAIN[0].min
					) {
						store.dispatch({
	...
	subscribeChannelName(ch: number, typeIndex: number, channelTypeIndex: number) {
		this.emberConnection
			.getNodeByPath(
				this.mixerProtocol.channelTypes[typeIndex].fromMixer.CHANNEL_NAME[0].mixerMessage.replace(
					'{channel}',
	...
	sendOutMessage(mixerMessage: string, channel: number, value: string | number, type: string) {
		...
		const message = mixerMessage.replace('{channel}', channelString)
	...
	sendOutLevelMessage(channel: number, value: number) {
		...
		const channelType = state.channels[0].channel[channel - 1].channelType
		const channelTypeIndex = state.channels[0].channel[channel - 1].channelTypeIndex

		if (channel < 25) {
			levelMessage = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_OUT_GAIN[0]
				.mixerMessage
			channelVal = 160 + channelTypeIndex + 1
		} else {
			levelMessage = this.mixerProtocol.channelTypes[channelType].toMixer.CHANNEL_OUT_GAIN[1]
				.mixerMessage
			channelVal = channelTypeIndex + 1
		}

		const valueNumber = value
		const valueByte = new Uint8Array([(valueNumber & 0x0000ff00) >> 8, valueNumber & 0x000000ff])
		const channelByte = new Uint8Array([channelVal & 0x000000ff])
		...
		const hexArray = levelMessage.split(' ')
...
*/
	...
	updateOutLevel(channelIndex: number) {
		...
		const outputlevel = state.channels[0].channel[channelIndex].outputLevel

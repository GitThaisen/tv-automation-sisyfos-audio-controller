...
export class QlClMixerConnection {
			...
			.on('data', (data: any) => {
				...
				buffers.forEach((message) => {
					...
					if (
						...
						const mixerValues: string[] = message.split(' ')
						const ch = parseInt(mixerValues[3])
						const assignedFader = 1 + state.channels[0].channel[ch - 1].assignedFader
						const mixerValue = parseInt(mixerValues[6])
						store.dispatch({
							type: SET_VU_LEVEL,
						...
						const ch = 1 + (message[11] | (message[10] << 8))
						const assignedFader = 1 + state.channels[0].channel[ch - 1].assignedFader
						const mixerLevel: number = message[16] | (message[15] << 8) // parseFloat(message[16])
						const faderLevel = Math.pow(2, mixerLevel / 1920) - 1
						...
						// MUTE ON/OFF COMMAND
						const channelIndex = message[11] | (message[10] << 8)

						const value: boolean = message[16] === 0 ? true : false
						logger.verbose(
	...
	sendOutMessage(message: string, channel: number, value: string | number, type: string) {
		...
		const valueByte = new Uint8Array([(valueNumber & 0xff00) >> 8, valueNumber & 0x00ff])

		const channelByte = new Uint8Array([(channel & 0xff00) >> 8, channel & 0x00ff])

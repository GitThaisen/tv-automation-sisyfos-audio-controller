...
export class MidiMixerConnection {
	...
	setupMixerConnection() {
		this.midiInput.addListener('controlchange', 1, (message: any) => {
			...
			if (
				message.data[1] >=
					parseInt(this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_OUT_GAIN[0].mixerMessage) &&
				message.data[1] <=
					parseInt(this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_OUT_GAIN[0].mixerMessage) +
						24
			) {
				const ch =
					...
					message.data[1] -
					parseInt(this.mixerProtocol.channelTypes[0].fromMixer.CHANNEL_OUT_GAIN[0].mixerMessage)
				const faderChannel = 1 + state.channels[0].channel[ch - 1].assignedFader
				store.dispatch({
...
*/
	...
	sendOutMessage(ctrlMessage: string, channel: number, value: string) {
		if (ctrlMessage != 'none' && 0 <= parseFloat(value) && parseFloat(value) <= 127) {
			const ctrlMessageInt = parseInt(ctrlMessage) + channel - 1
